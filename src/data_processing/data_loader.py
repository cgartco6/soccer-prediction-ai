...
class DataLoader:
    ...
    def create_data_generator(self, X: pd.DataFrame, y: pd.Series, 
        ...
        if X.empty or y.empty:
            self.logger.error("Cannot create generator: empty data")
            return
        
        if batch_size is None:
            # Calculate optimal batch size based on memory
            available_memory_mb = self.loading_config['max_memory_mb']
            
            # Estimate memory per sample
            sample_memory = X.memory_usage(deep=True).sum() / len(X) / 1024 / 1024  # MB per sample
            
            if sample_memory > 0:
                batch_size = int(available_memory_mb * 0.5 / sample_memory)  # Use 50% of available memory
                batch_size = max(32, min(batch_size, 1024))  # Between 32 and 1024
            else:
                batch_size = 64
        
        self.logger.info(f"Creating data generator with batch size {batch_size}")
        ...
        def generator():
            for start_idx in range(0, len(X), batch_size):
                ...
                X_batch = X.iloc[batch_indices].copy()
                y_batch = y.iloc[batch_indices].copy()
    ...
    def save_processed_data(self, data: Any, filename: str, 
        ...
        if use_compression is None:
            use_compression = self.loading_config['use_compression']
        
        filepath = self.processed_dir / filename
        
        try:
            if use_compression:
                filepath = filepath.with_suffix('.pkl.lz4')
    ...
    def _load_from_cache(self, key: str, as_dataframe: bool = False) -> Any:
        """Load data from cache"""
        if not self.loading_config['cache_enabled']:
            return None
        ...
        if cache_age > self.loading_config['cache_max_age_hours'] * 3600:
            try:
                cache_path.unlink()
            except:
                pass
        ...
        try:
            if self.loading_config['use_compression']:
    ...
    def cleanup_cache(self, max_age_hours: int = None):
        ...
        if max_age_hours is None:
            max_age_hours = self.loading_config['cache_max_age_hours'] * 2
    ...
    def get_data_report(self) -> Dict:
        ...
        report = {
            ...
            'file_counts': {},
            'cache_info': {
                'enabled': self.loading_config['cache_enabled'],
                'max_age_hours': self.loading_config['cache_max_age_hours'],
                'compression': self.loading_config['use_compression']
            }
        }
        
        # Count files in each directory
        for dir_name, directory in [('raw', self.raw_dir), 
                                   ('processed', self.processed_dir),
                                   ('cache', self.cache_dir)]:
            if directory.exists():
                files = list(directory.glob("*"))
                report['file_counts'][dir_name] = len(files)
                
                # Calculate total size
                total_size = sum(f.stat().st_size for f in files)
                report['file_counts'][f'{dir_name}_size_mb'] = total_size / (1024 * 1024)
